...
and directly connected to the spectragraph instrument. ~ Data will be
...
image data. ~ Version 2.0 of this test, when the spectragraph is on the
summit, will be that all data from the spectragraph will be written to
...
responsible to ingest the image file into the correct the ButlerG2 (or
G3) repository, and DBB repository. ~The L1 handoff system through the
OODS will enable quick access to images that were just created and keep
30 days worth of cache for scientists to view as needed from the summit
LSP. ~\\[2\baselineskip]
...
spectragraph. ~ OS is Centos 6.* running latest security patches. ~Cron
script to move to images NCSA installed on ATS storage server. ~ IP
tables and port verifications for data transfer to NCSA. ~Receiving end
at NCSA file systems available with DTN configured and protocol set up.
~ NCSA verification scripts including ButlerG* ingests.
~\\[3\baselineskip]
